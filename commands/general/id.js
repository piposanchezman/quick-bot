...
module.exports = class ID extends Command {
  ...
  async slashRun(interaction, args) {
    ...
    if(option == "channel") {
      interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.utility.getid.replace("<type>", `<#${value}>`).replace("<id>", value), this.client.embeds.success_color)], flags: this.client.cmdConfig.id.ephemeral ? Discord.MessageFlags.Ephemeral : 0 });
    } else if(option == "role") {
      interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.utility.getid.replace("<type>", `<@&${value}>`).replace("<id>", value), this.client.embeds.success_color)], flags: this.client.cmdConfig.id.ephemeral ? Discord.MessageFlags.Ephemeral : 0 });
    } else if(option == "user") {
      interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.utility.getid.replace("<type>", `<@${value}>`).replace("<id>", value), this.client.embeds.success_color)], flags: this.client.cmdConfig.id.ephemeral ? Discord.MessageFlags.Ephemeral : 0 });
